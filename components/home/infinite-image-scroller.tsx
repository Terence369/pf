      ...
      >
        <div className="inline-flex" style={{ willChange: "transform", justifyContent: 'flex-start', alignItems: 'flex-start', paddingTop: '104px' }}>
          {imgs.concat(imgs).map((src, i) => (
            ...
            >
              <img src={src} alt={`scroller-${i}`} className="w-full h-full object-cover transform-gpu will-change-transform" style={{ transform: 'scale(1.06)', transition: 'transform 600ms ease' }} loading="lazy" decoding="async" />
            </div>
          ))}
        </div>
      </div>
